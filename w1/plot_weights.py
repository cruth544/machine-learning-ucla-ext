...
def pla_plot(weights):
    ...
    set_a = pd.read_excel('coin_data/coin_data_a.xlsx', 'Sheet1', header=None)
    set_b = pd.read_excel('coin_data/coin_data_b.xlsx', 'Sheet1', header=None)
    ...
    # I won't try to answer in full right now. However..let's just say for now that 'the separator'..whatever it may
    # be....exists in one higher dimension that where the data is sitting.
